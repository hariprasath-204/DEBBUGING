...
const AdminDashboard = () => {
  const [activeTab, setActiveTab] = useState('questions'); // 'questions' | 'event' | 'tracker'
      ...
      <div style={{ flex: 1, overflow: 'hidden' }}>
        {activeTab === 'questions' && (
          <div className="glass-panel" style={{ padding: '2rem', height: '100%', overflowY: 'auto' }}>
            <h2 style={{ color: 'var(--accent-pink)', marginBottom: '1.5rem' }}>ADD NEW QUESTION</h2>
        ...
        {activeTab === 'event' && (
          <div className="glass-panel" style={{ padding: '2rem', height: '100%' }}>
            <h2 style={{ color: 'var(--accent-pink)', marginBottom: '1.5rem' }}>EVENT CONTROLS</h2>
            <div style={{ marginBottom: '2rem' }}>
              <h3 style={{ color: 'var(--text-secondary)' }}>CURRENT STATUS: <span className={eventStatus === 'active' ? 'glow-text-cyan' : 'glow-text-pink'} style={{ fontSize: '1.5rem', marginLeft: '1rem', textTransform: 'uppercase' }}>{eventStatus}</span></h3>
            </div>
            ...
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-end', marginBottom: '2rem' }}>
              ...
              <button onClick={handleStopEvent} disabled={eventStatus !== 'active'} className="btn-secondary" style={{ background: 'var(--accent-pink)', color: 'var(--bg-deep-navy)' }}>STOP EVENT</button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'tracker' && (
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem', overflowY: 'auto', paddingRight: '0.5rem' }}>
              {liveUsers.map(user => (
                <div key={user.id} className="glass-panel" style={{ padding: '1rem', border: (user.tabSwitches > 2 || user.copyPasteCount > 2) ? '1px solid var(--accent-pink)' : '1px solid var(--border-subtle)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '0.5rem', marginBottom: '0.5rem' }}>
                    ...
                    <span style={{ color: user.isFinished ? 'var(--accent-pink)' : 'var(--text-secondary)' }}>{user.isFinished ? 'SUBMITTED' : 'CODING'}</span>
                  </div>
                  
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
                    <span>Tab Switches: <strong style={{ color: user.tabSwitches > 0 ? 'var(--accent-pink)' : 'inherit' }}>{user.tabSwitches || 0}</strong></span>
                    <span>Copy/Paste: <strong style={{ color: user.copyPasteCount > 0 ? 'var(--accent-pink)' : 'inherit' }}>{user.copyPasteCount || 0}</strong></span>
                  </div>
